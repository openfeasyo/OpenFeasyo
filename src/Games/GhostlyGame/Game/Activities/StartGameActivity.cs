    ...
    public class StartGameActivity : OpenFeasyo.GameTools.UI.Activity
    {
        public StartGameActivity(UIEngine engine) : base(engine)
        {
            ...
            if (GameSessionInfo.Instance.LevelsCompleted < GameSessionInfo.Instance.RequiredLevels)
            {
                ...
                float tileWidth = (engine.Screen.ScreenWidth * 0.9f) / 6;
                float tileHeight = (engine.Screen.ScreenHeight * 0.75f) / 5;

                LevelSelectionButton StartGameButton = new LevelSelectionButton(LocalizationResourceManager.Instance["StartToPlay"].ToString(), engine.Content.LoadFont(GhostlyGame.MENU_BUTTON_FONT + GhostlyGame.MENU_BUTTON_FONT_SIZE), engine.Device);
                ...
                StartGameButton.Position = (new Vector2(engine.Screen.ScreenMiddle.X - (tileWidth), engine.Screen.ScreenMiddle.Y - (tileHeight)));
            ...
            else
            {
                ...
                sessionCompleted.Position = new Vector2(engine.Screen.ScreenMiddle.X - (sessionCompleted.Size.X / 2), cell * 4);
                ...
                closeGame.Position = new Vector2(engine.Screen.ScreenMiddle.X - (sessionCompleted.Size.X / 2), cell * 6);
