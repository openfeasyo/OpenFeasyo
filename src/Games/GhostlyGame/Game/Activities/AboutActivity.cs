...
namespace GhostlyLib.Activities
{
    public class AboutActivity : OpenFeasyo.GameTools.UI.Activity
    {
        public AboutActivity(UIEngine engine) : base(engine)
        {
            ...
            Label delucaLabel = new Label("Application development was supported by the De Luca Foundation and Delsys, Inc.", engine.Content.LoadFont("Fonts/Ubuntu" + fontSize), GhostlyGame.MENU_FONT_COLOR);
            delucaLabel.Position = new Vector2(engine.Screen.ScreenMiddle.X, cell * 3) - delucaLabel.Size / 2;

            Label musicHeaderLabel = new Label("Music in this game", engine.Content.LoadFont("Fonts/Ubuntu" + headerSize), GhostlyGame.MENU_FONT_COLOR);
            musicHeaderLabel.Position = new Vector2(engine.Screen.ScreenMiddle.X, cell * 5) - musicHeaderLabel.Size / 2;

            Label music1Label = new Label("Music from https://filmmusic.io", engine.Content.LoadFont("Fonts/Ubuntu" + fontSize), GhostlyGame.MENU_FONT_COLOR);
            music1Label.Position = new Vector2(engine.Screen.ScreenMiddle.X, cell * 6) - music1Label.Size / 2;

            Label music2Label = new Label("\"Wholesome\" and \"Arcadia\" by Kevin MacLeod(https://incompetech.com)", engine.Content.LoadFont("Fonts/Ubuntu" + fontSize), GhostlyGame.MENU_FONT_COLOR);
            music2Label.Position = new Vector2(engine.Screen.ScreenMiddle.X, cell * 7) - music2Label.Size / 2;

            Label music3Label = new Label("License: CC BY (http://creativecommons.org/licenses/by/4.0/)", engine.Content.LoadFont("Fonts/Ubuntu" + fontSize), GhostlyGame.MENU_FONT_COLOR);
            music3Label.Position = new Vector2(engine.Screen.ScreenMiddle.X, cell * 8) - music3Label.Size / 2;

            Label authorsHeaderLabel = new Label("Authors", engine.Content.LoadFont("Fonts/Ubuntu" + headerSize), GhostlyGame.MENU_FONT_COLOR);
            authorsHeaderLabel.Position = new Vector2(engine.Screen.ScreenMiddle.X, cell * 10) - authorsHeaderLabel.Size / 2 ;

            Label authors1Label = new Label("This game was developed at Vrije Universiteit Brussel (ETRO dept.) by", engine.Content.LoadFont("Fonts/Ubuntu" + fontSize), GhostlyGame.MENU_FONT_COLOR);
            authors1Label.Position = new Vector2(engine.Screen.ScreenMiddle.X, cell * 11) - authors1Label.Size / 2;

            Label authors2Label = new Label("Katarina Kostkova and Lubos Omelina in the team of prof. Bart Jansen", engine.Content.LoadFont("Fonts/Ubuntu" + fontSize), GhostlyGame.MENU_FONT_COLOR);
