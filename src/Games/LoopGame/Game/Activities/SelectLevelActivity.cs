...
namespace LoopLib.Activities
{
    public class SelectLevelActivity : OpenFeasyo.GameTools.UI.Activity
    {
        public SelectLevelActivity(UIEngine engine, int world) : base(engine) {
            ...
            Label infoLabel = new Label("Select the level", engine.Content.LoadFont(LoopGame.MENU_BUTTON_FONT + LoopGame.MENU_BUTTON_FONT_SIZE), LoopGame.MENU_FONT_COLOR);
            infoLabel.Position = new Vector2(engine.Screen.ScreenMiddle.X - (infoLabel.Size.X / 2), engine.Screen.ScreenHeight * 0.10f - (LoopGame.MENU_BUTTON_FONT_SIZE/2));
            ...
            for (int y = 0; y < 5; y++)
                for (int x = 0; x < 6; x++) {
                    LevelSelectionButton level1Button = new LevelSelectionButton(levelNum.ToString(), engine.Content.LoadFont(LoopGame.MENU_BUTTON_FONT + LoopGame.MENU_BUTTON_FONT_SIZE), engine.Device);
